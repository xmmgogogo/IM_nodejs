var net         = require('net'),
    ...
    redisClient = redis.createClient({ "host": "127.0.0.1", "port": "6370" }),
    initSharedMemory = require('./node_modules/sharedmemory').init;
...
var HOST        = '0.0.0.0',
    ...
    // 缓存数据 | 数据缓存KEY
    USERMAP                                 = 'USERMAP',        // 用户map {uid => {'UserId' => 'lmm', 'PlatformId':'101'}}
    GROUPLIST                               = 'GROUPLIST',      // 频道分组 {'group1' => [1,2,3], 'group2' : []}
...
function imLogin(myClientSocket, imType, body) {
    ...
    userInfo.GroupIds       = [];
    userInfo.ExtInfo        = '';
    addUserId(userId, userInfo);

    //3，记录统计信息
    //getAllUserListInfo(function(error, getAllUserInfo) {
    //    ConnectCount = Object.keys(getAllUserInfo).length;
...
function joinGroup(myClientSocket, imType, body) {
    ...
    // 将频道id写入用户数据
    addUserGroup(uId, groupId);

    // 将用户数据写入group
    // 若频道不存在，则创建一个频道
    addGROUPLIST(uId, groupId);

    // stat 更新统计数目-频道数
    getGROUPLISTLength(groupId, function(error, maxCount) {
...
function quitGroup(myClientSocket, imType, body) {
    ...
    // 删除用户数据里面的频道
    delUserGroup(uId, groupId);

    // 删除频道用户ID
    delGROUPLISTByUid(uId, groupId);
...
function serverResponseStatus(myClientSocket, imType, body, extra) {
    ...
    global.log("[serverResponseStatus - success]: [" + imType + "]" +  util.inspect(body));
...
function SendError(socket, errorCode, errorMsg) {
    ...
    global.log("[serverResponseStatus - Error]: " +  util.inspect(body));
 ...
 * @param callFun
 */
function getUserInfo(uId, callFun) {
    hgetCacheData(USERMAP, uId, callFun);
}

function addUserId(uId, userInfo) {
    try {
        // 这里的做法，直接覆盖即可
        hsetCacheData(USERMAP, uId, userInfo)
    } catch (e) {
        global.log(e);
    }
}

function delUserId(uId) {
    //直接删除
    hdelCacheData(USERMAP, uId)
}
 ...
 * @param groupId
 */
function addUserGroup(uId, groupId) {
    // 将频道id写入用户数据
    hgetCacheData(USERMAP, uId, function(error, UserInfo) {
        if(UserInfo && UserInfo['GroupIds'].indexOf(groupId) < 0) {
            UserInfo['GroupIds'].push(groupId);
            hsetCacheData(USERMAP, uId, UserInfo)
        }
    });
}
 ...
 * @param groupId
 */
function delUserGroup(uId, groupId) {
    // 删除用户数据里面的频道
    hgetCacheData(USERMAP, uId, function(error, UserInfo) {
        if(UserInfo) {
            var d_u_i = UserInfo['GroupIds'].indexOf(groupId);
            if(d_u_i >= 0) {
                delete UserInfo['GroupIds'][d_u_i];
                hsetCacheData(USERMAP, uId, UserInfo)
            }
        }
    });
 ...
 * @param callFun
 */
function getAllUserListInfo(callFun) {
    hgetallCacheData(USERMAP, callFun);
}
 ...
 * @param callFun
 */
function getGROUPLIST(groupId, callFun) {
    try {
        var key = getGroupCacheKey(groupId);
        hgetallCacheData(key, callFun);
    } catch (e) {
        global.log(e);
    }
}

function addGROUPLIST(uId, groupId) {
    try {
        var key = getGroupCacheKey(groupId);
        hsetCacheData(key, uId, 1);
    } catch (e) {
        global.log(e);
    }
}

function delGROUPLIST(groupId) {
    var key = getGroupCacheKey(groupId);
    delCacheData(key);
}

function delGROUPLISTByUid(uId, groupId) {
    var key = getGroupCacheKey(groupId);
    hdelCacheData(key, uId);
}
 ...
 * @param callFun
 */
function getGROUPLISTLength(groupId, callFun) {
    hgetallCacheData(getGroupCacheKey(groupId), function(error, data) {
        var len = 0;
        if(data) {
...
function getGroupCacheKey(groupId) {
    return GROUPLIST + ':' + groupId;
}

/**********************基础缓存结构 - REDIS*************************************/

function hgetCacheData(key, subkey, callFun) {
    // redis 发现缓存object有问题，但转string后，socket数据报错
    redisClient.hget(key, subkey, function(error, data) {
        data = JSON.parse(data);
        callFun(error, data);
    });

    //sharedMemoryController.get(key, function(data) {global.log('sharedMemoryController.get ' + subkey);global.log(data);
    //    if(data && data.hasOwnProperty(subkey)) {
    //        callFun(0, data[subkey]);
    //    }
    //});
}

function hsetCacheData(key, subkey, value) {
    value = JSON.stringify(value);
    redisClient.hset(key, subkey, value);

    //sharedMemoryController.get(key, function(res) {
    //    var data = {};
    //    if(res) {
    //        data = res;
    //    }
    //
    //    data[subkey] = value;
    //    sharedMemoryController.set(key, data);
    //});
}

function hdelCacheData(key, subkey, callFun) {
    redisClient.hdel(key, subkey, callFun);
...
function hgetallCacheData(key, callFun) {
    redisClient.hgetall(key, callFun);
...
function delCacheData(key) {
    ...
    //sharedMemoryController.remove(key);
}
...
function imCheckOnline(clientSocket, body) {
    ...
    // 事件回调型处理方式
    getUserInfo(UserId, function(error, data) {
        if(data) {
            body['onLine'] = 1;
        } else {
            body['onLine'] = 0;
...
function imKickUser(myClientSocket, imType, body) {
    ...
    getUserInfo(UserId, function(error, userInfo) {
        if(userInfo) {
            // 直接发送结果
            send(userInfo['Conn'], IM_KICK_USER, IM_FROM_TYPE_SYSTEM, {msg : messageBody});
        }
...
function imKickAll(myClientSocket, imType, body) {
    ...
    getAllUserListInfo(function(error, getAllUserInfo) {
        if(getAllUserInfo) {
            for(UserId in getAllUserInfo) {
                if(getAllUserInfo.hasOwnProperty(UserId)) {
                    // 直接发送结果
                    send(getAllUserInfo[UserId]['Conn'], IM_KICK_USER, IM_FROM_TYPE_SYSTEM, {msg : messageBody});
                }
            }
        }
...
function imGroupUserList(myClientSocket, body) {
    ...
    getGROUPLIST(groupId, function(error, data) {
        if(data) {
            // 直接发送结果
            send(myClientSocket, IM_KICK_USER, IM_FROM_TYPE_SYSTEM, {userList : data});
        }
    });
}
...
function imGroupIdList(myClientSocket) {
    // 直接发送结果
    getAllUserListInfo(function(error , data) {
        send(myClientSocket, IM_KICK_USER, IM_FROM_TYPE_SYSTEM, {idList : Object.keys(data)});
    });
}
...
function imChatBoardCast(myClientSocket, imType, fromType, body) {
    try {
        ...
        getUserInfo(UserId, function(error, senderInfo) {
            if(senderInfo) {
                //...
            ...
            // 生成完整包数据
            // 生成完整包数据
            var data = VA_formatMsgHeader(imType, 0, body);

            // 遍历所有在线用户，发送消息
            getAllUserListInfo(function(error, getAllUserInfo) {
                for(var uId in getAllUserInfo) {
                    if(getAllUserInfo.hasOwnProperty(uId)) {
                        var Conn = getAllUserInfo[uId]['Conn'];
                        if(Conn) {
                            Conn.write(data);
                        }
                    } else {
...
function imChatGroup(myClientSocket, imType, fromType, body) {
    try {
        ...
        getUserInfo(UserId, function(error, senderInfo) {
            ...
            // 遍历当前频道全部用户，发送消息
            getGROUPLIST(groupId, function(error, result) {
                for(var sendUid in result) {
                    if(result.hasOwnProperty(sendUid)) {
                        getUserInfo(sendUid, function(error, res) {
                            var Conn = res['Conn'];
                            if(Conn) {
                                Conn.write(data);
                            }
...
function imChatPrivate(myClientSocket, imType, fromType, body) {
    try {
        ...
        // 接受者信息
        getUserInfo(receiverId, function(error , receiverInfo) {
            if(receiverInfo) {
                //...
            } else {
        ...
        function onReceivePackData(buffer) {
            ...
            if(receive_data) {
                try {
                    ...
                    global.log("Receive Data: [" + imType + ']' + "[" + fromType + "]" + util.inspect(body));
