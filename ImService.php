...
$socket = ImService::get();
$socket->connect();
$socket->sendSystem(['type' => 1, 'msg' => "abcdef"]);
...
class ImService
{
     ...
     * @return ImService
     */
    public static function get($host = "172.17.5.168", $port = "6969", $systemKey = "XXOOOOXX")
    {
        if (!self::$instance) {
    ...
    private function read()
    {
        ...
        // 读取协议长度
        $length = current(unpack("n", socket_read($this->socket, 2)));
        $socketData['length'] = $length;
        // 读取消息类型
        $socketData['imType'] = current(unpack("n", socket_read($this->socket, 2)));
        // 读取来源类型
        $socketData['fromType'] = current(unpack("n", socket_read($this->socket, 2)));
        // 读取协议内容
        $socketData['body'] = json_decode(socket_read($this->socket, $length-4));
